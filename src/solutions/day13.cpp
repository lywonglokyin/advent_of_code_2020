...
class Day13 : public Solution {
   public:
    void q1() override {
        ...
        std::cout << "q1: " << earliest_bus_id * shortest_wait_time
                  << std::endl;
    }
    void q2() override {
        auto input = utils::parse_file_text("./input/day13.txt");
        ...
        std::cout << "q2: "
                  << least_common_multiple_with_modulo_recursive(parsedInput)
                  << std::endl;
    }

   private:
    long long least_common_multiple_with_modulo_recursive(
        const std::vector<std::pair<long long, long long>>& input) {
        // Assume the divisors of the input are distinct
        if (input.size() < 2) {
            throw - 1;
        }
        if (input.size() == 2) {
            return least_common_multiple_with_modulo(
                input[0].first, input[0].second, input[1].first,
                input[1].second);
        }
        ...
        for (auto pair : input) {
            ...
            long long target = least_common_multiple_with_modulo(
                pair.first, pair.second, max_divisor, max_divisor_modulo);
            long long shift_count = (target - max_divisor_modulo) / max_divisor;
            long long indifferent_shifts =
                utils::lcm(pair.first, max_divisor) / max_divisor;
            new_input.emplace_back(
                std::make_pair(indifferent_shifts, shift_count));
        }

        long long to_shift =
            least_common_multiple_with_modulo_recursive(new_input);
        return to_shift * max_divisor + max_divisor_modulo;
    }

    long long least_common_multiple_with_modulo(long long a, long long mod_a,
                                                long long b, long long mod_b) {
        // std::cout<< "Calling normal with: Divide " << a <<" remains " <<
        // mod_a <<
        // ", Divide " <<b << " remains " <<mod_b << "." <<std::endl;
        if (a > b) {
            std::swap(a, b);
        ...
        int total_shift_amount = least_common_multiple_with_modulo(
            a_shift_amount, 0, a, a_need_to_shift);
