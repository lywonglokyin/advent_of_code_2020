...
class Bag {
   public:
    ...
    Bag(std::string name) { this->name = name; }
   ...
   public:
    void q1() override {
        ...
        for (auto line : input) {
            ...
            std::string count = sections[0].substr(
                containPos + 8, spacePos - (containPos + 7) - 1);
            std::string child =
                sections[0].substr(spacePos + 1, bagPos - spacePos - 2);
