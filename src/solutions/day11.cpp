...
class Day11 : public Solution {
    ...
    void q2() override {
        ...
        while (changed) {
            ...
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    char new_c =
                        get_updated_char_with_visibility(input, Pos{x, y});
                    if (new_c != input[y][x]) {
                        changed = true;
                        to_be_update.push(std::make_pair(Pos{x, y}, new_c));
                    }
   ...
   private:
    char get_updated_char(const std::vector<std::string>& input,
                          const Pos& pos) {
    ...
    char get_updated_char_with_visibility(const std::vector<std::string>& input,
                                          const Pos& pos) {
        char my_val = input[pos.y][pos.x];
        if (my_val == '.') {
            return '.';
    ...
    inline int get_surrounding_occupied_seat(
        const std::vector<std::string>& input, const Pos& pos) {
        int width = input[0].length();
        int height = input.size();
        int count = 0;
    ...
    inline int get_visible_occupied_seat(const std::vector<std::string>& input,
                                         const Pos& pos) {
        int width = input[0].length();
        int height = input.size();
        int count = 0;
