...
class Day14 : public Solution {
   ...
   private:
    std::vector<unsigned long> get_combination(unsigned long base,
        ...
        if (index == positions.size() - 1) {
            return std::vector<unsigned long>{base & zero_mask,
                                              base | one_mask};
        } else {
            std::vector<unsigned long> result_zero =
                get_combination(base & zero_mask, positions, index + 1);
            std::vector<unsigned long> result_one =
                get_combination(base | one_mask, positions, index + 1);
