...
public:
    void q1() override{
        ...
        bool valid[BOOL_ARRAY_SIZE] = {false};  // Use fixed size array to indicate if a value is valid.
        ...
        for (size_t i = 0; i<=FIELD_RULE_LINE_END; ++i){
            ...
            for (int j = std::stoi(first_range_pair.first); j <= std::stoi(first_range_pair.second); ++j){
                valid[j] = true;
            }
            auto second_range_pair = utils::split_as_pair(range_pair.second.substr(2), '-');
            for (int j = std::stoi(second_range_pair.first); j <= std::stoi(second_range_pair.second); ++j){
                valid[j] = true;
            }
        }
        ...
        for (size_t i = NEARBY_TICKET_LINE_START; i<input.size(); ++i){
            ...
            for (int num: nearby_ticket_nums){
                if ((num<0) || (num>=BOOL_ARRAY_SIZE) || !valid[num]){
                    sum += num;
                }
            }
        }
    ...
    void q2() override{
        ...
        bool not_possible[NUM_OF_FIELDS][NUM_OF_FIELDS] = {false};  // e.g. [0][1] = true means the 0th col could not be the 1st field.
        ...
        for (size_t i = NEARBY_TICKET_LINE_START; i<input.size(); ++i){
            auto nearby_ticket = utils::split(input[i], ',');
            auto nearby_ticket_nums = utils::stringvec_to_intvec(nearby_ticket);
            for (size_t col_idx = 0; col_idx < nearby_ticket_nums.size(); ++col_idx){
        ...
        std::array<int, NUM_OF_FIELDS> field_num = {0};  // e.g [2] refers to the field num of the 2nd col.
        bool changed = true;
        while(changed){
            changed = false;
            for (size_t col_idx=0; col_idx<NUM_OF_FIELDS; ++col_idx){
                ...
                if (possible_count == 1){
                    changed = true;
                    for (size_t col_idx_inner = 0; col_idx_inner<NUM_OF_FIELDS; ++col_idx_inner){
            ...
            for (size_t field_idx=0; field_idx<NUM_OF_FIELDS; ++field_idx){
                ...
                if (possible_count == 1){
                    changed = true;
                    for (size_t field_idx_inner = 0; field_idx_inner<NUM_OF_FIELDS; ++field_idx_inner){
