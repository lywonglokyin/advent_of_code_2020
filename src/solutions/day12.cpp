...
struct Ship {
    enum Direction { N = 0, E, S, W, NUM_DIRECTION };
    ...
    void turnRight(int degree) {
        d = static_cast<Direction>((d + degree / 90) %
    ...
    void moveNorth(int units) { y += units; }
    void moveSouth(int units) { y -= units; }
    void moveEast(int units) { x += units; }
    void moveWest(int units) { x -= units; }
};
...
struct WaypointShip {
    ...
    void moveNorth(int units) { waypoint.y += units; }
    void moveSouth(int units) { waypoint.y -= units; }
    void moveEast(int units) { waypoint.x += units; }
    void moveWest(int units) { waypoint.x -= units; }
};
