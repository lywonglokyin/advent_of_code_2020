...
public:
    void q1() override{
        ...
        for (std::string s: input){
            ...
            int max = std::stoi(s.substr(dash_pos+1, colon_pos - dash_pos - 2));
    ...
    void q2() override{
        ...
        for (std::string s: input){
            ...
            int pos2 = std::stoi(s.substr(dash_pos+1, colon_pos - dash_pos - 2));
            char target = s.at(colon_pos - 1);
            std::string password = s.substr(colon_pos+2);

            if ((password.at(pos1-1) == target) != (password.at(pos2-1) == target)){
                ++valid;
            }
        }
