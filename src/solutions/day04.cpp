...
class Day4 : public Solution {
   public:
    void q1() override {
        ...
        size_t count = 0;
        bool passport_checklist[PassportFields::NUMBER_OF_PASSPORT_FIELDS] = {
            false};
        for (auto line : input) {
            if (line == "") {
                ...
                for (size_t i = 0;
                     i < PassportFields::NUMBER_OF_PASSPORT_FIELDS; ++i) {
    ...
    void q2() override {
        ...
        for (auto line : input) {
            if (line == "") {
                if (is_passport_value_valid(passport_checklist,
                                            passport_value)) {
                    ++count;
                }
                for (size_t i = 0;
   ...
   private:
    bool is_passport_valid(
        bool checklist[PassportFields::NUMBER_OF_PASSPORT_FIELDS]) {
    ...
    bool is_passport_value_valid(
        bool checklist[PassportFields::NUMBER_OF_PASSPORT_FIELDS],
        std::string values[PassportFields::NUMBER_OF_PASSPORT_FIELDS]) {
        if (!checklist[PassportFields::BYR] ||
            !validBYR(values[PassportFields::BYR]))
            return false;
        if (!checklist[PassportFields::IYR] ||
            !validIYR(values[PassportFields::IYR]))
            return false;
        if (!checklist[PassportFields::EYR] ||
            !validEYR(values[PassportFields::EYR]))
            return false;
        if (!checklist[PassportFields::HGT] ||
            !validHGT(values[PassportFields::HGT]))
            return false;
        if (!checklist[PassportFields::HCL] ||
            !validHCL(values[PassportFields::HCL]))
            return false;
        if (!checklist[PassportFields::ECL] ||
            !validECL(values[PassportFields::ECL]))
            return false;
        if (!checklist[PassportFields::PID] ||
            !validPID(values[PassportFields::PID]))
            return false;
        return true;
    }
    ...
    bool validECL(std::string value) {
        std::vector<std::string> possibleValues{"amb", "blu", "brn", "gry",
                                                "grn", "hzl", "oth"};
        bool contains = false;
